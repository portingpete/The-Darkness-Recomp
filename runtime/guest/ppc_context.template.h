...
inline bool PPCIsDynamicMmio(uint32_t address) {
    return (address >= 0x7FC00000u && address < 0x7FD00000u) ||
           (address >= 0x7FE00000u && address < 0x7FF00000u);
}
inline bool PPCIsInvalidGuestAddress(uint32_t address) {
    return address < 0x10000u;
}
inline void PPCTraceInvalidGuestLoad(uint32_t address, unsigned width) {
    static std::atomic<uint32_t> count{};
    if (count.fetch_add(1, std::memory_order_relaxed) < 32)
        fprintf(stderr, "[MEM] low guest load width=%u address=0x%08X\n", width, address);
}
inline uint8_t PPCLoadU8(uint8_t* base, uint32_t address) {
    if (PPCIsInvalidGuestAddress(address)) { PPCTraceInvalidGuestLoad(address, 1); return 0; }
    return PPCIsDynamicMmio(address) ? uint8_t(0) : *(volatile uint8_t*)(base + address);
}
inline uint16_t PPCLoadU16(uint8_t* base, uint32_t address) {
    if (PPCIsInvalidGuestAddress(address)) { PPCTraceInvalidGuestLoad(address, 2); return 0; }
    return PPCIsDynamicMmio(address) ? uint16_t(0) : __builtin_bswap16(*(volatile uint16_t*)(base + address));
}
inline uint32_t PPCLoadU32(uint8_t* base, uint32_t address) {
    if (PPCIsInvalidGuestAddress(address)) { PPCTraceInvalidGuestLoad(address, 4); return 0; }
    return PPCIsDynamicMmio(address) ? 0u : __builtin_bswap32(*(volatile uint32_t*)(base + address));
}
inline uint64_t PPCLoadU64(uint8_t* base, uint32_t address) {
    if (PPCIsInvalidGuestAddress(address)) { PPCTraceInvalidGuestLoad(address, 8); return 0; }
    return PPCIsDynamicMmio(address) ? 0ull : __builtin_bswap64(*(volatile uint64_t*)(base + address));
}
#ifndef PPC_LOAD_U8
#define PPC_LOAD_U8(x) PPCLoadU8(base, uint32_t(x))
#endif
#ifndef PPC_LOAD_U16
#define PPC_LOAD_U16(x) PPCLoadU16(base, uint32_t(x))
#endif
#ifndef PPC_LOAD_U32
#define PPC_LOAD_U32(x) PPCLoadU32(base, uint32_t(x))
#endif
#ifndef PPC_LOAD_U64
#define PPC_LOAD_U64(x) PPCLoadU64(base, uint32_t(x))
#endif

inline void PPCStoreU32(uint8_t* base, uint32_t address, uint32_t value) {
    if (!PPCIsInvalidGuestAddress(address) && !PPCIsDynamicMmio(address))
        *(volatile uint32_t*)(base + address) = __builtin_bswap32(value);
}
inline void PPCStoreU64(uint8_t* base, uint32_t address, uint64_t value) {
    if (!PPCIsInvalidGuestAddress(address) && !PPCIsDynamicMmio(address))
        *(volatile uint64_t*)(base + address) = __builtin_bswap64(value);
}
#ifndef PPC_STORE_U32
#define PPC_STORE_U32(x, y) PPCStoreU32(base, uint32_t(x), uint32_t(y))
#endif
#ifndef PPC_STORE_U64
#define PPC_STORE_U64(x, y) PPCStoreU64(base, uint32_t(x), uint64_t(y))
#endif
// Xenon MMIO is not guest RAM. Until a renderer owns these registers,
// return reset values for reads and discard writes so translated hardware
// paths remain observable without faulting on reserved address ranges.
#ifndef PPC_MM_LOAD_U8
#define PPC_MM_LOAD_U8(x)  (uint8_t(0))
#endif

#ifndef PPC_MM_LOAD_U16
#define PPC_MM_LOAD_U16(x) (uint16_t(0))
#endif

#ifndef PPC_MM_LOAD_U32
#define PPC_MM_LOAD_U32(x) (uint32_t(0))
#endif

#ifndef PPC_MM_LOAD_U64
#define PPC_MM_LOAD_U64(x) (uint64_t(0))
#endif

#ifndef PPC_STORE_U8
#define PPC_STORE_U8(x, y) do { if (!PPCIsDynamicMmio(uint32_t(x))) *(volatile uint8_t*)(base + (x)) = (y); } while (0)
#endif

#ifndef PPC_STORE_U16
#define PPC_STORE_U16(x, y) do { if (!PPCIsDynamicMmio(uint32_t(x))) *(volatile uint16_t*)(base + (x)) = __builtin_bswap16(y); } while (0)
#endif
...
// MMIO writes are ignored until renderer register emulation is available.
#ifndef PPC_MM_STORE_U8
#define PPC_MM_STORE_U8(x, y)   do { (void)(x); (void)(y); } while (0)
#endif

#ifndef PPC_MM_STORE_U16
#define PPC_MM_STORE_U16(x, y)  do { (void)(x); (void)(y); } while (0)
#endif

#ifndef PPC_MM_STORE_U32
#define PPC_MM_STORE_U32(x, y)  do { (void)(x); (void)(y); } while (0)
#endif

#ifndef PPC_MM_STORE_U64
#define PPC_MM_STORE_U64(x, y)  do { (void)(x); (void)(y); } while (0)
#endif
...
// A missing target is a translation/runtime failure, never a successful call.
inline void PPCSafeIndirect(struct PPCContext& __restrict ctx, uint8_t* base, uint32_t guest);
#define PPC_CALL_INDIRECT_FUNC(x) PPCSafeIndirect(ctx, base, (uint32_t)(x))
#endif

[[noreturn]] void PPCRecompFailure(const struct PPCContext& ctx, uint32_t address, const char* reason);
uint64_t PPCQueryTimebase();
#define PPC_QUERY_TIMEBASE() PPCQueryTimebase()
#define PPC_RECOMP_FAILURE(ctx, address, reason) PPCRecompFailure(ctx, address, reason)
#define PPC_TRACE_ENTRY(ctx, address) do { (ctx).lastFunction = (address); (ctx).trace[(ctx).traceIndex++ & 63] = (address); } while (0)
...
inline void PPCSafeIndirect(struct PPCContext& __restrict ctx, uint8_t* base, uint32_t guest) {
    if ((guest & 3) == 0 && guest >= PPC_CODE_BASE && guest < (PPC_CODE_BASE + PPC_CODE_SIZE)) {
        PPCFunc* f = PPC_LOOKUP_FUNC(base, guest);
        if (f) {
            f(ctx, base);
            return;
        }
    }
    PPC_RECOMP_FAILURE(ctx, guest, "missing or invalid indirect function target");
}
